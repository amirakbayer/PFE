...
export class ProgressComponent
  ...
  constructor(protected override progressHelper: ProgressHelperService, private etat: EtatService) {
    super(progressHelper);
    this.Etats=this.etat.etatNames();
  }
  ...
  protected generateProgressArray(length): any[] {
    return [...Array(length).keys()].map((key) => {
      return {
        stepIndex: key,
        status: key === this.activeIndex ? Status.IN_PROGRESS : key < this.activeIndex? Status.COMPLETED : Status.PENDING,
      };
    });
  }
